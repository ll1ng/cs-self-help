...
#define insert2unsorted(p)                                                                                             \
    ({                                                                                                                 \
        ...
        if (free_pointer_list[0])                                                                                      \
            SET_BACK(free_pointer_list[0], p);                                                                         \
...
void *mm_malloc(size_t size)
{
    ...
    if (!bp && ((bp = extend_heap(extendsize / WSIZE)) == NULL))
        return NULL;

    SET_HF(bp, size, 1);
#if request_debug
    printf("\e[31;49;1mmalloc: %p\nsize: %d 0x%x\nalloc: %d\n----------------------\n\e[39;49;0m", bp, GET_SIZE(bp),
...
#if request_debug
    printf("\033[1;33mrealloc: %p\nsize: %d 0x%x\nalloc: %d\n-----------------\n\e[39;49;0m", newptr - DSIZE,
           GET_SIZE(newptr - DSIZE), GET_SIZE(newptr - DSIZE), GET_ALLOC(newptr - DSIZE));
#endif
    return newptr;
}
...
static void *search4list(int size)
{
    ...
    else
    {
        ...
        {
            if (idx != 5)
                unsorted = &free_pointer_list[MAX_FASTBIN_COUNT + idx];
            else
                unsorted = &free_pointer_list[0];
            current = *unsorted;
            while (current != NULL)
            {
                // split to 2 chunks
...
void showbins()
{
    ...
    for (idx = 0; idx <= MAX_FASTBIN_COUNT + UNSORTED_COUNT; idx++)
    {
        ...
        if (idx == 0)
            printf("(>0x3000)idx [%d]:", idx);
        else if (idx <= MAX_FASTBIN_COUNT)
            printf("(0x%x)idx [%d]:", idx << 3, idx);
        else
        {
